// ==UserScript==
// @name         Ficha Individual Analítica JVGemini (Pendências por Disciplina)
...
// --- FUNÇÃO MODIFICADA: Alunos para Recomposição ---
function AlunosParaRecomposicao(dataArray, item, titulo) {
    // Diálogo de confirmação exigido
    let confirmar = confirm("Aviso Importante:\n\nCertifique-se de que a Ficha Individual da Turma está configurada com a situação 'Matriculados' (e NÃO 'Matriculados Ativos') antes de exportar.\n\nDeseja prosseguir com a exportação?");
    if (!confirmar) return;
    ...
    // Realiza a ordenação estrita por ordem alfabética do Nome do Aluno
    ...
    alunosOrdenados.forEach(function([cod, aluno]) {
        let row = [];
        row.push(cod);
        row.push(aluno.dadosAluno.nome);
        row.push(aluno.resultado);

        let precisaRecomposicao = false;
        let rowNotas = [];

        // Alunos transferidos não entram na lógica ativa de notas vermelhas
        if (aluno.resultado === "TRANSFERIDO DA ESCOLA" || aluno.resultado === "TRANSFERIDO DA TURMA") {
            precisaRecomposicao = false;
            for (var i = 0; i < dataArray.materias.length; i++) {
                rowNotas.push("-");
            }
        } else {
            for (var i = 0; i < dataArray.materias.length; i++) {
                var materia = dataArray.materias[i];
                var valor = '-';

                if (aluno.notas[materia] != undefined && aluno.notas[materia][item] != undefined) {
                    valor = aluno.notas[materia][item];
                }
        ...
        // Se o aluno não tiver nenhuma nota menor que 6, preenche o lugar das notas com a informação solicitada
        if (!precisaRecomposicao) {
            for (var j = 0; j < rowNotas.length; j++) {
                rowNotas[j] = "Não necessita de recomposição";
            }
        }

        row = row.concat(rowNotas);
        ws_data.push(row);
    });
    ...
    var ws = XLSX.utils.aoa_to_sheet(ws_data);
    var wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Recomposição");
    XLSX.writeFile(wb, nomeArquivo);
}
...
function MapaParaAprovacao(dataArray) {
    ...
    Object.keys(dataArray.alunos).forEach(function(cod) {
        let row = [];
        let pointsAcumulados = new Array(dataArray.materias.length).fill(0);
        ...
        for (var i = 0; i < dataArray.materias.length; i++) {
             ...
             if (materiaNotas != undefined) {
                 ...
                 pointsAcumulados[i] = finalN1 + finalN2 + finalN3 + finalN4;
             }
        }
        ...
        for (var u = 0; u < dataArray.materias.length; u++) {
            var mediaApr = (24 - (pointsAcumulados[u])).toFixed(2);
            if (mediaApr >= 24){ row.push("-"); }
...
(function() {
    ...
    // --- SEÇÃO: Alunos para Recomposição (< 6) ORDENADO ----------------------
    var ttttRecomp = document.createElement('div');
    ttttRecomp.textContent = 'Alunos para Recomposição (< 6.0) ⬇️';
    ttttRecomp.style.backgroundColor= '#242420';
    ttttRecomp.className = "menu-item";
    ttttRecomp.addEventListener('click', function() {
        if(optRecomp.style.display == 'none'){
            optRecomp.style.display = 'block';
            ttttRecomp.textContent = 'Alunos para Recomposição (< 6.0) ⬆️';
        }else{
            optRecomp.style.display = 'none';
            ttttRecomp.textContent = 'Alunos para Recomposição (< 6.0) ⬇️';
        }
    });
    menuContainer.appendChild(ttttRecomp);

    var optRecomp = document.createElement('div');
    optRecomp.style.display = 'none';

    var opcaosRecomp = [ "Recomposição 1º Bimestre", "Recomposição 2º Bimestre", "Recomposição 3º Bimestre", "Recomposição 4º Bimestre"];
    var varivaRecomp = [ "N1", "N2", "N3", "N4"];
    for (var r = 0; r < opcaosRecomp.length; r++) {
        (function(nome, vvv) {
            let btn = document.createElement('div');
            btn.className = "menu-item";
            btn.textContent = "Excel: " + nome;
            btn.addEventListener('click', function() {
                AlunosParaRecomposicao(infos, vvv, nome);
            });
            optRecomp.appendChild(btn);
        })(opcaosRecomp[r], varivaRecomp[r]);
    }
    menuContainer.appendChild(optRecomp);
    menuContainer.appendChild(document.createElement('hr'));
    // --------------------------------------------------------------------------
    ...
    // --- SEÇÃO: Disciplinas com pendências (Agrupado) ---------------------
    ...
    for (var d = 0; d < opcaosDisc.length; d++) {
        (function(nome) {
            ...
            subButton.textContent = nome;
            subButton.addEventListener('click', function() {
                pendenciasPorDisciplina(vvv, infos);
            });
