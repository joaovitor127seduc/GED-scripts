...
// --- FUNÇÃO: Alunos para Recomposição (Notas < 6) ORDENADO ALFABETICAMENTE ---
function AlunosParaRecomposicao(dataArray, item, titulo) {
    ...
    alunosOrdenados.forEach(function([cod, aluno]) {
        ...
        // Ignorar transferidos para a busca ativa de recomposição
        if (aluno.resultado === "TRANSFERIDO DA ESCOLA" || aluno.resultado === "TRANSFERIDO DA TURMA") return;
        ...
        for (var i = 0; i < dataArray.materias.length; i++) {
            ...
            // Verifica se a matéria e a nota existem para o aluno
            ...
            if (!stringas.includes(valor)) {
                ...
                // Verifica se é um número válido e estritamente abaixo de 6
                if (!isNaN(nota) && nota < 6) {
                    row.push(nota);
                    precisaRecomposicao = true;
                } else {
                    row.push("-"); // Notas azuis (>= 6) ficam em branco/traço
                }
            } else {
                row.push("-"); // Sem lançamento ou string especial ficam em branco/traço
            }
        }

        // Adiciona a linha na planilha SOMENTE se o aluno tiver pelo menos uma nota vermelha lançada
    ...
    if (ws_data.length === 1) {
        alert(`Parabéns! Nenhum aluno com nota lançada abaixo de 6 no ${titulo}.`);
        return;
    }
